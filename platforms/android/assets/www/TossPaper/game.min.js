...
function prepareMIBand(deviceAddress,deviceName,isConnected){
						var miband = new BC.Device({deviceAddress:deviceAddress,deviceName:deviceName,type:'BLE',isConnected:isConnected});
						miband.discoverServices(function(){
							miband.services[2].discoverCharacteristics(function(){
								setTimeout(function(){
									var firstErrorData = true;
									miband.services[2].characteristics[13].subscribe(function(data){
										...
										if(datalen == 16 || datalen == 28){
											...
											preyy = yy;
											prezz = zz;									
										}else if(firstErrorData){
											firstErrorData = false;
											console.log("data length is not right : "+datalen);
											miband.disconnect(function(){
												prepareMIBand(miband.deviceAddress,miband.deviceName,true);
											},function(){});
										}
									}
								)},100);
						...
						}, "BCBluetooth", "setNotification", [{"deviceAddress":miband.deviceAddress,"serviceIndex":2,"characteristicIndex":13,"enable":"true"}]); 
						
						setTimeout(function(){	
							var value = BC.Tools.HexToBase64("1201");
							
							cordova.exec(function(){
								xiaomiisready = true;
							},function(){
								alert("write start notify error!!!");
							}, "BCBluetooth", "writeValue", [{"deviceAddress":miband.deviceAddress,"serviceIndex":2,"characteristicIndex":4,"descriptorIndex":"","writeValue":value}]);							
						},100);
						
						setInterval(function(){
							var value1 = BC.Tools.HexToBase64("1200");
								
							cordova.exec(function(){
								var value2 = BC.Tools.HexToBase64("1201");
								
								cordova.exec(function(){
								},function(){
									alert("write start notify error!!!");
								}, "BCBluetooth", "writeValue", [{"deviceAddress":miband.deviceAddress,"serviceIndex":2,"characteristicIndex":4,"descriptorIndex":"","writeValue":value2}]);	
							},function(){
								alert("write start notify error!!!");
							}, "BCBluetooth", "writeValue", [{"deviceAddress":miband.deviceAddress,"serviceIndex":2,"characteristicIndex":4,"descriptorIndex":"","writeValue":value1}]);	
						},60000);*/	
}
...
document.addEventListener('bcready', function () {
			...
			BC.Bluetooth.GetConnectedDevices(function(devices){
				...
				for(var i = 0;i < devices.length;i++){
					if(devices[i].deviceName == "MI"){
						isxiaomiband = true;
						prepareMIBand(devices[i].deviceAddress,devices[i].deviceName,true);
					}
				}
        	...
        	/*BC.bluetooth.addEventListener("newadvpacket", function (arg) {
				var scanData = arg.target;
				...
				if (newDevice.RSSI > -50) {
        			...
        			newDevice.connect(function () {
        				newDevice.prepare(function () {
							...
							setInterval(function(){
								newDevice.services[2].characteristics[4].write("Hex","1200",function(){
									newDevice.services[2].characteristics[4].write("Hex","1201",function(){
...
MLE.Loader = function (a, b) {
    ...
    for (c = 0; c < a.length; c++)
        ...
        d.onLoad = this.faceFontLoaded.bindd(this);
        d.loadFont(a[c].src)
    ...
    e.addProgressListener(this.onProgress.bindd(this));
    e.addCompletionListener(this.onComplete.bindd(this));
    e.start()
};
...
MouseInput.prototype = {
    ...
    startListening: function () {
        this.useTouch === true ? (MLE.wrapper.addEventListener("touchstart", this.touchStart.bindd(this), false), MLE.wrapper.addEventListener("touchmove", this.touchMove.bindd(this), false), MLE.wrapper.addEventListener("touchend", this.touchEnd.bindd(this), false)) : (MLE.wrapper.addEventListener("mousedown", this.onMouseDown.bindd(this), false), MLE.wrapper.addEventListener("mousemove",
            this.onMouseMove.bindd(this), false), MLE.wrapper.addEventListener("mouseup", this.onMouseUp.bindd(this), false))
    },
...
PreLoaderScene.prototype.init = function () {
    ...
    //this.eyeAnimFA.addAnim("blink", [0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3], 20);
    this.eyeAnimFA.addAnim("blink", [0,0], 20);
...
PreLoaderScene.prototype.update = function () {
    ...
    if (this.percent === 100 && this.loaded === false && xiaomiisready === true) this.loaded = true, createjs.Tween.get(this).wait(2E3).call(function () {
        MLE.start()
    })
};
...
MainMenu.prototype.init = function () {
    ...
    this.moreGames.clickCallback = this.moreGamesClicked.bindd(this);*/
    var a = 0;
    ...
    this.easyButton.clickCallback = this.easyButtonClicked.bindd(this);
    ...
    this.mediumButton.clickCallback = this.mediumButtonClicked.bindd(this);
    ...
    this.hardButton.clickCallback = this.hardButtonClicked.bindd(this);
    this.active = false;
...
MainMenu.prototype.update = function () {
    this.active && (this.playButton.update(), this.easyButton.update(), this.mediumButton.update(), this.hardButton.update())
};
...
GameScene.prototype.update = function () {
    ...
    if (!this.paused) {
        ...
        var a = (xiaomishoot) && this.ball.sprite.x === this.ball.startPosX && this.ball.sprite.y === this.ball.startPosY && this.aimArrow.sprite.alpha >= 0.75;
        this.shadow.x = this.ball.sprite.x;
        this.aimArrow.update();
        if (a && this.currentState === this.states.IDLE) xiaomishoot=false,this.currentState = this.states.AIMING;
        else if (a && this.currentState === this.states.AIMING) xiaomishoot=false,this.ball.makeShoot(this.aimArrow.shotDirectionX, this.aimArrow.shotDirectionY, this.aimArrow.shootAngle), this.makeShadowTween(), this.ball.rotateFast(), this.currentState = this.states.BEFORE_WIND, this.aimArrow.stopRotationTween();
...
WindCloud.prototype.init = function () {
    ...
    this.numberPin = new MLE.Pin(this.windFont, this.group, getConst(40), 30);
    this.windNumber = 2.5;
    this.tween = {}
};
