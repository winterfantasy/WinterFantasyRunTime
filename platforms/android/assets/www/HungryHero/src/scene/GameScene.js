...
var GameScene = cc.Scene.extend({
    ...
    _handleHeroPose:function() {
        ...
        if (Math.abs(-(this._hero.y - this._touchY) * 1.6) < 30)
            this._hero.setRotation((this._hero.y - this._touchY) * 1.6);
        //modified by ron
        //if(Math.abs(ax)>0.05)
        	//this._hero.setRotation(ax*300);
    ...
    /**
     * hero琚鎾濶娆″悗锛岀粨鏉熸父鎴忥紱缁撴潫涔嬪墠锛屽厛鎾斁hero鎺夎惤鐨勫姩鐢�     */
    endGame:function(){
        this.x = 0;
        this.y = 0;
    ...
    update:function(elapsed) {
        var winSize = cc.director.getWinSize();
        this._touchY=this._hero.y-ax*150;ax=0;
        switch(Game.gameState){
            ...
            case GameConstants.GAME_STATE_FLYING:
                ...
                if (Game.user.hitObstacle <= 0) {
                    this._hero.y -= (this._hero.y - this._touchY) * 0.8;
                	//modified by ron
                	//this._hero.y-=ax*100;ax=0;
